...
var debug = debug_1.default('EventedHTTPServer');
...
var EventedHTTPServer = /** @class */ (function (_super) {
    ...
    function EventedHTTPServer() {
        ...
        _this.listen = function (targetPort) {
            ...
            _this._tcpServer.on('listening', function () {
                ...
                if (address && typeof address !== 'string') {
                    ...
                    _this.emit(EventedHTTPServerEvents.LISTENING, port);
                }
            });
        ...
        _this._onConnection = function (socket) {
            ...
            connection.on(EventedHTTPServerEvents.REQUEST, function (request, response, session, events) { _this.emit(EventedHTTPServerEvents.REQUEST, request, response, session, events); });
            connection.on(EventedHTTPServerEvents.ENCRYPT, function (data, encrypted, session) { _this.emit(EventedHTTPServerEvents.ENCRYPT, data, encrypted, session); });
            connection.on(EventedHTTPServerEvents.DECRYPT, function (data, decrypted, session) { _this.emit(EventedHTTPServerEvents.DECRYPT, data, decrypted, session); });
            connection.on(EventedHTTPServerEvents.CLOSE, function (events) { _this._handleConnectionClose(connection, events); });
            _this._connections.push(connection);
        };
        _this._handleConnectionClose = function (connection, events) {
            _this.emit(EventedHTTPServerEvents.SESSION_CLOSE, connection.sessionID, events);
...
var Session = /** @class */ (function (_super) {
    ...
    function Session(connection) {
        ...
        _this._connectionDestroyed = function () {
            ...
            _this.emit(HAPSessionEvents.CLOSED);
        };
    ...
    function EventedHTTPServerConnection(server, clientSocket) {
        ...
        _this.sendEvent = function (event, data, contentType, excludeEvents) {
            ...
            if (_this._writingResponse) {
                ...
                _this.emit(EventedHTTPServerEvents.ENCRYPT, data, encrypted, _this._session);
                if (encrypted.data) {
                    // @ts-ignore
                    data = encrypted.data;
                }
        ...
        _this._sendPendingEvents = function () {
            ...
            _this._pendingEventData.forEach(function (event) {
                var encrypted = { data: null };
                _this.emit(EventedHTTPServerEvents.ENCRYPT, event, encrypted, _this._session);
                if (encrypted.data) {
                    // @ts-ignore
        ...
        _this._onClientSocketData = function (data) {
            ...
            _this.emit(EventedHTTPServerEvents.DECRYPT, data, decrypted, _this._session);
            if (decrypted.error) {
        ...
        _this._onHttpServerRequest = function (request, response) {
            ...
            _this.emit(EventedHTTPServerEvents.REQUEST, request, response, _this._session, _this._events);
        };
        _this._onHttpServerClose = function () {
            ...
            _this.emit(EventedHTTPServerEvents.CLOSE, _this._events);
        };
