...
var AccessoryInfo = /** @class */ (function () {
    function AccessoryInfo(username) {
        ...
        this._removePairedClient0 = function (controller, username) {
            if (_this.pairedClients[username] && _this.pairedClients[username].permission === PermissionTypes.ADMIN)
                _this.pairedAdminClients--;
            delete _this.pairedClients[username];
            if (Object.keys(_this.pairedClients).length == 0) {
                _this.relayEnabled = false;
                _this.relayState = 2;
                _this.relayAccessoryID = "";
                _this.relayAdminID = "";
                _this.relayPairedControllers = {};
                _this.accessoryBagURL = "";
            }
        ...
        this.updateRelayEnableState = function (state) {
            _this.relayEnabled = state;
        };
        this.updateRelayState = function (newState) {
            _this.relayState = newState;
        };
        this.addPairedRelayClient = function (username, accessToken) {
            _this.relayPairedControllers[username] = accessToken;
        };
        this.removePairedRelayClient = function (username) {
            delete _this.relayPairedControllers[username];
        };
        this.save = function () {
            var saved = {
                ...
                relayEnabled: _this.relayEnabled,
                relayState: _this.relayState,
                relayAccessoryID: _this.relayAccessoryID,
                relayAdminID: _this.relayAdminID,
                relayPairedControllers: _this.relayPairedControllers,
                accessoryBagURL: _this.accessoryBagURL
            };
            for (var username in _this.pairedClients) {
                var pairingInformation = _this.pairedClients[username];
                //@ts-ignore
            ...
            node_persist_1.default.setItemSync(key, saved);
            node_persist_1.default.persistSync();
        };
        this.remove = function () {
            var key = AccessoryInfo.persistKey(_this.username);
            node_persist_1.default.removeItemSync(key);
        };
    ...
    AccessoryInfo.load = function (username) {
        ...
        var saved = node_persist_1.default.getItem(key);
        if (saved) {
