...
var debug = debug_1.default('DataStream:Server');
var Protocols;
(function (Protocols) {
...
var DataStreamServer = /** @class */ (function (_super) {
    ...
    function DataStreamServer() {
        ...
        _this.state = ServerState.UNINITIALIZED;
    ...
    DataStreamServer.prototype.prepareSession = function (session, controllerKeySalt, callback) {
        ...
        var accessoryToControllerEncryptionKey = hkdf.HKDF("sha512", salt, session.encryption.sharedSec, DataStreamServer.accessoryToControllerInfo, 32);
        var controllerToAccessoryEncryptionKey = hkdf.HKDF("sha512", salt, session.encryption.sharedSec, DataStreamServer.controllerToAccessoryInfo, 32);
        var preparedSession = {
            session: session,
    ...
    DataStreamServer.prototype.handleMessageGlobally = function (connection, message) {
        var _a;
        assert_1.default.notStrictEqual(message.type, MessageType.RESPONSE); // responses can't physically get here
        var separator = "";
        var args = [];
        ...
        var hadListeners;
        try {
            hadListeners = (_a = this.internalEventEmitter).emit.apply(_a, [message.protocol + separator + message.topic, connection].concat(args));
        }
        catch (error) {
            hadListeners = true;
...
var DataStreamConnection = /** @class */ (function (_super) {
    ...
    function DataStreamConnection(socket) {
         ...
         */
        _this.state = ConnectionState.UNIDENTIFIED;
        ...
        _this.addProtocolHandler(Protocols.CONTROL, {
            requestHandler: (_a = {},
                _a[Topics.HELLO] = _this.handleHello.bind(_this),
                _a)
        });
    ...
    DataStreamConnection.prototype.onSocketData = function (data) {
        ...
        if (this.state === ConnectionState.UNIDENTIFIED) {
            ...
            this.emit(DataStreamConnectionEvents.IDENTIFICATION, firstFrame, function (identifiedSession) {
                if (identifiedSession) {
                    ...
                    _this.state = ConnectionState.EXPECTING_HELLO;
                    _this.session.on(eventedhttp_1.HAPSessionEvents.CLOSED, _this.onHAPSessionClosed.bind(_this)); // register close listener
                }
            });
            if (this.state === ConnectionState.UNIDENTIFIED) {
        ...
        messages.forEach(function (message) {
            if (message.type === MessageType.RESPONSE) {
                ...
                if (handler === undefined) {
                    ...
                    _this.emit(DataStreamConnectionEvents.HANDLE_MESSAGE_GLOBALLY, message);
                    return;
                }
                if (message.type === MessageType.EVENT) {
    ...
    DataStreamConnection.prototype.decryptHDSFrame = function (frame, keyOverwrite) {
        ...
        if (encryption.verifyAndDecrypt(key, this.controllerToAccessoryNonceBuffer, frame.cipheredPayload, frame.authTag, frame.header, frame.plaintextPayload)) {
    ...
    DataStreamConnection.prototype.sendHDSFrame = function (header, message) {
        ...
        var cipheredPayload = Buffer.alloc(payloadBuffer.length);
