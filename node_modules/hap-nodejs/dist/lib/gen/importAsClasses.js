...
var simple_plist_1 = __importDefault(require("simple-plist"));
...
var metadata = simple_plist_1.default.readFileSync(plistPath);
// begin writing the output file
var outputPath = path_1.default.join(__dirname, '..', '..', '..', 'src', 'lib', 'gen', 'HomeKitTypes.generated.ts');
var output = fs_1.default.createWriteStream(outputPath);
...
function getCharacteristicFormatsKey(format) {
    ...
    for (var key in Characteristic_1.Formats)
        if (Characteristic_1.Formats[key] == format)
            return key;
...
function getCharacteristicUnitsKey(units) {
    ...
    for (var key in Characteristic_1.Units)
        if (Characteristic_1.Units[key] == units)
            return key;
...
//# sourceMappingURL=importAsClasses.js.map