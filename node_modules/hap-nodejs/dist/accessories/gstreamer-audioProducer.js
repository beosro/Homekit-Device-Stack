...
var StreamController_1 = require("../lib/StreamController");
var index_1 = require("../index");
var debug = debug_1.default("Remote:GStreamer");
var AudioType;
(function (AudioType) {
...
var GStreamerAudioProducer = /** @class */ (function () {
    ...
    GStreamerAudioProducer.prototype.startAudioProduction = function (selectedAudioConfiguration) {
        ...
        assert_1.default(selectedAudioConfiguration.codecType === StreamController_1.AudioCodecTypes.OPUS);
        var bitrateType = BitrateType.VARIABLE;
        switch (codecParameters.bitrate) {
            case StreamController_1.AudioCodecParamBitRateTypes.CONSTANT:
                bitrateType = BitrateType.CONSTANT;
                break;
            case StreamController_1.AudioCodecParamBitRateTypes.VARIABLE:
                bitrateType = BitrateType.VARIABLE;
                break;
        }
        var bandwidth = Bandwidth.SUPER_WIDE_BAND;
        switch (codecParameters.samplerate) {
            case StreamController_1.AudioCodecParamSampleRateTypes.KHZ_8:
                bandwidth = Bandwidth.NARROW_BAND;
                break;
            case StreamController_1.AudioCodecParamSampleRateTypes.KHZ_16:
                bandwidth = Bandwidth.WIDE_BAND;
                break;
            case StreamController_1.AudioCodecParamSampleRateTypes.KHZ_24:
        ...
        this.process.on("error", function (error) {
            if (_this.running) {
                ...
                _this.errorHandler(index_1.DataSendCloseReason.CANCELLED);
            }
            else {
        ...
        this.process.on("exit", function (code, signal) {
            if (signal !== "SIGTERM") { // if we receive SIGTERM, process exited gracefully (we stopped it)
                ...
                _this.errorHandler(index_1.DataSendCloseReason.UNEXPECTED_FAILURE);
            }
        });
    };
